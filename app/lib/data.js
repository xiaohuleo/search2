...
// 辅助函数：生成模拟的高频访问量 (符合长尾分布，少数非常高，多数普通)
function getRandomVisits() {
  const rand = Math.random();
  if (rand > 0.95) return Math.floor(Math.random() * 5000000) + 100000; // 5% 爆款 (10w - 500w)
  if (rand > 0.8) return Math.floor(Math.random() * 100000) + 10000;   // 15% 热门 (1w - 10w)
  return Math.floor(Math.random() * 5000) + 100;                       // 80% 普通 (100 - 5000)
}
...
export const DEFAULT_DATA = RAW_TEXT.split(/，|\n/)
  ...
  .map((name, index) => {
    ...
    return {
      ...
      "发布渠道": "移动端,PC端",
      "满意度": getRandomScore(),
      "访问量": getRandomVisits().toString() // 新增核心字段：访问量
    };
  });
